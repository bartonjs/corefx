...
namespace System.Security.Cryptography.Tests.Asn1
{
    public class WriteOctetString : Asn1WriterTests
    {
        ...
        [Fact]
        public void WriteOctetString_DER_PrimitiveOnly()
        {
            AsnWriter writer = new AsnWriter(AsnEncodingRules.DER);

            Assert.Throws<CryptographicException>(
                () => writer.WriteOctetString(new Asn1Tag(UniversalTagNumber.OctetString, true), ReadOnlySpan<byte>.Empty));

            Assert.Throws<CryptographicException>(
                () => writer.WriteOctetString(new Asn1Tag(TagClass.ContextSpecific, 4, true), ReadOnlySpan<byte>.Empty));
        }
