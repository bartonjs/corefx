...
Ji0iy6T3Y16v8maAqNihK6YdWZI19n2ctNWPF4PTykPnjwpauqYkB5k2wMOp");
            ...
            using (ECDsa ecdsa = ECDsaFactory.Create())
            {
                Assert.ThrowsAny<CryptographicException>(
                    () => ecdsa.ImportSubjectPublicKeyInfo(explicitSPKI, out _));

                Assert.ThrowsAny<CryptographicException>(
                    () => ecdsa.ImportECPrivateKey(explicitECPrivateKey, out _));

                Assert.ThrowsAny<CryptographicException>(
                    () => ecdsa.ImportPkcs8PrivateKey(explicitPkcs8, out _));
                ...
                Assert.ThrowsAny<CryptographicException>(
                    () => ecdsa.ImportEncryptedPkcs8PrivateKey("asdf", explicitEncryptedPkcs8, out _));
            }
        }
...
ctLI7vH2zDIF0AV+ud5sqeMQUJY=",
        ...
        {
            using (ECDsa key = ECDsaFactory.Create())
            {
                int bytesRead = -1;
            ...
            in ECParameters expected)
        {
            ReadWriteKey(
                ...
                (ECDsa ecdsa, Span<byte> destination, out int bytesWritten) =>
                    ecdsa.TryExportEncryptedPkcs8PrivateKey(passwordBytes, pbe, destination, out bytesWritten),
                isEncrypted: true);
        }
        ...
        private static void ReadWriteBase64ECPrivateKey(string base64Pkcs8, in ECParameters expected)
        {
            ReadWriteKey(
                ...
                (ECDsa ecdsa, Span<byte> destination, out int bytesWritten) =>
                    ecdsa.TryExportECPrivateKey(destination, out bytesWritten));
        }
